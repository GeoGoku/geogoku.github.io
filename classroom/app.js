...
async function studentHome(){
  ...
  app.innerHTML=`<section class="card"><p class="eyebrow">学生端</p><h1>加入课堂</h1><p>点选你的班级，填写姓名和学号。</p><form id="join"><input type="hidden" id="session" name="sessionId" value="${requestedSession>0?requestedSession:''}"><div class="field"><span class="label-text">选择课堂</span><div id="session-list" class="session-list"><p class="muted">正在查找课堂…</p></div></div><div class="field"><label for="manual-session">课堂编号（列表出不来时填写）</label><input id="manual-session" inputmode="numeric" pattern="[0-9]*" placeholder="老师屏幕上的编号，例如 3" maxlength="10"></div><div class="field"><label for="name">姓名</label><input id="name" name="name" value="${esc(identity.name||'')}" autocomplete="name" maxlength="40" required></div><div class="field"><label for="studentId">学号</label><input id="studentId" name="studentId" value="${esc(identity.studentId||'')}" autocomplete="off" maxlength="40" required></div><button class="full" id="join-button">进入课堂</button></form><p id="connection" class="muted" role="status" aria-live="polite">正在连接答题服务…</p><button type="button" class="secondary full" id="retry-sessions">重新查找课堂</button><p id="session-help" class="info" hidden>手机打不开课堂列表时，把老师屏幕上的课堂编号填进去即可。用系统浏览器打开本页通常比微信内置浏览器更稳。</p><p class="info">请核对班级。作答时间结束后不能补交；每题提交后不能修改。</p></section><p class="muted">题目请看老师的 PPT，此处只需选择答案。</p>`;
  $('#join').onsubmit=startAction(async e=>{
    const b=Object.fromEntries(new FormData(e.currentTarget));
    const typed=($('#manual-session')?.value||'').trim();
    if(typed)b.sessionId=typed;
    if(!b.sessionId)throw new Error('请先点选班级，或填写老师给出的课堂编号。');
...
function renderSessionList(sessions, selected){
  const box=$('#session-list'), hidden=$('#session');
  if(!box||!hidden)return;
  if(!sessions.length){
    box.innerHTML='<p class="muted">暂时没有进行中的课堂。可填写下面的课堂编号。</p>';
    return;
  }
  if(!selected && requestedSession && sessions.some(s=>s.id===requestedSession)) selected=requestedSession;
  if(!selected && sessions.length===1) selected=sessions[0].id;
  hidden.value=selected||'';
  box.innerHTML=sessions.map(s=>`<button type="button" class="session-choice${s.id===Number(selected)?' selected':''}" data-session="${s.id}"><strong>${esc(s.className)}</strong><span>${esc(s.course)} · 编号 ${s.id}</span></button>`).join('');
  box.querySelectorAll('[data-session]').forEach(btn=>btn.onclick=()=>{
    hidden.value=btn.dataset.session;
    box.querySelectorAll('.session-choice').forEach(el=>el.classList.toggle('selected',el===btn));
  });
}
async function pollSessions(){
  if(sessionId||teacher||sessionPollPending||!$('#session-list'))return;
  clearTimeout(refreshTimer);
  ...
  try{
    ...
    if(sessionId||!$('#session-list'))return;
    const nextSignature=JSON.stringify(data.sessions.map(s=>[s.id,s.className,s.course]));
    if(nextSignature!==sessionListSignature){
      renderSessionList(data.sessions, Number($('#session').value)||0);
      sessionListSignature=nextSignature;
    }
    $('#session-help').hidden=!!data.sessions.length;
    connection(true);
    if(!data.sessions.length)$('#connection').textContent='已连接答题服务，目前没有进行中的课堂。';
  }catch(e){
    delay=8000;
    if(sessionId||!$('#session-list'))return;
    if(sessionListSignature===null)$('#session-list').innerHTML='<p class="error">课堂列表暂时打不开。请填写老师屏幕上的课堂编号，或点重新查找。</p>';
    $('#session-help').hidden=false;
    connection(false,'课堂列表获取失败。'+e.message);
  }finally{
    sessionPollPending=false;
    if(retry?.isConnected)retry.disabled=false;
    if(!sessionId&&!teacher&&$('#session-list'))schedule(pollSessions,delay);
  }
}
...
function resumePoll(){if(document.hidden||busy)return;if(teacher&&sessionId)pollTeacher();else if(!teacher&&sessionId)pollStudent();else if(!teacher)pollSessions();}document.addEventListener('visibilitychange',resumePoll);window.addEventListener('pageshow',resumePoll);

...
function failText(m){
  m=String(m||'');
  if(m==='network'||m==='timeout'||m.indexOf('连不上')>=0)return '进入失败：4G 连不上课堂接口。请改连校园网，或用 FreeCDN 加速后再试。';
  if(m.indexOf('访问来源')>=0)return '微信拦截了课堂接口。请点右上角「…」→「在浏览器打开」后再进入。';
...
function load(){
  return new Promise(function(ok){
    ...
    function fin(){
      ...
      if(left>0)return;
      var list=[];
      if(worker&&worker.sessions&&worker.sessions.length) list=worker.sessions;
      else if(local&&local.sessions&&local.sessions.length) list=local.sessions;
      else if(worker&&worker.sessions) list=worker.sessions||[];
      ok({sessions:list, err:(!list.length && werr)?werr:''});
    }
...
function joinUI(list,err){
  ...
  if(list.length){
    ...
    cards='<p class="err">手机连不上课堂列表。请填编号 6（地物23-1）后点进入。</p>';
  }else{
    cards='<p class="err">列表是空的。老师若已开课，直接填课堂编号 6 即可。</p>';
  }
  app.innerHTML='<section class="card"><h1>加入课堂</h1><p>点班级卡片；没有卡片就填编号 <b>6</b>。微信里请点右上角「在浏览器打开」。</p><div id="list">'+cards+'</div><form id="f"><input type="hidden" id="sid" value="'+(pick||'')+'"><label>课堂编号</label><input id="manual" inputmode="numeric" placeholder="老师屏幕上的数字" value="'+(pick||want||6)+'"><label>姓名</label><input id="name" required value="'+esc(ident.name||'')+'"><label>学号</label><input id="stu" required value="'+esc(ident.studentId||'')+'"><button>进入课堂</button></form><p id="msg" hidden></p><button type="button" class="ghost" id="retry">重新查找</button><p class="cdnfoot"><a target="_blank" href="http://www.freecdn.pw/?zzwz">本站由免费云加速（FreeCDN）提供网站加速和攻击防御服务</a></p></section>';
...
function poll(){if(!sid)return;api('/api/state',null,1).then(function(d){state=d;svc('已连通');draw();if(!d.session.ended)setTimeout(poll,2000);}).catch(function(e){svc('未连通');var b=document.getElementById('box');if(b)b.innerHTML='<p class="err">'+esc(failText(e.message))+'</p>';setTimeout(poll,4000);});}
...
function boot(){joinUI([],'');var list=document.getElementById('list');if(list)list.innerHTML='<p>正在查找班级…</p>';load().then(function(d){joinUI(d.sessions||[],d.err||'');}).catch(function(e){joinUI([],e.message);});}
